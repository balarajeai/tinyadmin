...
- **Status:** Proposed — Security **re-review** requested after SEC-PR12-001..003 remediation (CRITICAL)
- **Date:** 2026-09-19
...
3. **Canonicalization contract (normative):** Canonical mutation payload is **UTF-8 JSON** produced by **RFC 8785 JSON Canonicalization Scheme (JCS)**. The object **MUST** contain exactly these keys (sorted by JCS): `action_or_field_op` (discriminated object identifying Action id **or** approved-field operation), `targets` (structured record selector(s); never raw SQL/Mongo text), `parameters` (requested values), `max_affected_records` (integer; required — use Action/config limit). Unknown keys forbidden in V1 canonical object (fail closed). Binary/non-JSON values are not permitted in the canonical object; encode as JSON strings/numbers/arrays/objects only.
...
4. On reconnect, Agent **MUST** re-establish current authorization/session state and process cancels / validity before executing any pending mutates.
