...
**Status:** Proposed — **Security review required** (CRITICAL); Independent Code Review after Security  
...
Additional §5.6.1 mapping: durable Agent result store + retry until Cloud `result_ack`; Cloud lifecycle `pending` / `succeeded` / `failed` / `unknown`; reconciliation on reconnect.
...
sequenceDiagram
  ...
  participant Agent
  Act->>Act: Confirm gate passed; mint signed authz envelope (operation_id, …, exp)
  Act->>OB: Insert command (pending_dispatch) keyed by operation_id + agent_id
  alt Agent online
    OB->>WS: Push command frame
    WS->>Agent: command + signed envelope + payload
    Agent->>Agent: Verify Cloud sig; check org/env/agent/connection/exp; dedupe operation_id
  else Agent offline
    Note over OB: Wait for reconnect; enforce TTL/depth
  end
```

**Command frame (logical fields — not wire schema freeze for #6):**

- `message_type`: `command`
- `command_type`: `discover` \| `search` \| `preview` \| `execute` \| `rollback` \| …
- `authorization`: signed envelope (§3.5 fields)
- `payload`: command-specific structured params (**never** arbitrary SQL/Mongo text)
- `protocol_version`

**Expiry / TTL:** Agent rejects `exp` in the past. Cloud cancels outbox rows past TTL and marks operation `failed` or leaves `unknown` only when execution may have occurred without result (see §8).

**Duplicate commands:** Agent idempotency store wins; return stored terminal result if any.
...
sequenceDiagram
  ...
  Cloud->>Op: Correlate; transition pending→succeeded/failed; idempotent if duplicate
  Cloud->>Cloud: Append audit terminal (or unknown path)
  Cloud-->>Agent: result_ack(operation_id)
  Agent->>Local: Delete or mark acked
  Note over Agent,Cloud: If no ack: retry with backoff indefinitely within retention; never drop mutate result silently
```

**HTTPS fallback:** `POST /agent/v1/results` with Agent signature over body, same idempotency rules — required so result delivery is not stranded on WS-only failures.
...
sequenceDiagram
  ...
  participant Agent
  Admin->>Cloud: Revoke agent_id
  Cloud->>Cloud: agent=revoked; invalidate sessions
  Cloud->>OB: Cancel queued mutates for agent_id
  Cloud->>Cloud: Audit revoke + cancellations
  Note over Agent: Next frame/authn fails; in-flight mutate without valid session must not start; if already mutated, result retry may fail authn → Cloud ops path marks unknown for human/ops reconciliation
```

Also: permission/Action revoke cancels matching queued mutates (SR-QUEUE-REAUTH) even if Agent still valid.
...
| Duplicate command delivery | Agent idempotent by `operation_id` |
...
| Replay attack | Signature + `operation_id` dedupe + `exp` |
...
## 11. Ordering, backpressure, observability
...
| Metrics (Cloud) | Connected Agents; outbox depth; command age; result lag; authn failures; reject reasons |
| Metrics (Agent) | Connected; unacked results; last heartbeat; local dedupe size |
...
- [ ] At-least-once command delivery; exactly-once mutate effect via `operation_id` idempotency persisted across Agent restart
...
| Item | Owner | Notes |
| --- | --- | --- |
| Exact envelope codec (JSON + detached sig vs compact token) | Architect tweak OK in impl spike; Security review of final codec | Logical fields frozen here |
